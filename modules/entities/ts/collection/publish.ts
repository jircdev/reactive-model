...
export class CollectionSaveManager {
  ...
  sync = async () => {
    try {
      await this.#localProvider.init();
      const data = await this.#parent.localProvider.store.filter(record => record.offline === true).toArray();

      console.log("dataaaaaaa", data);
      const chunks = await this.splitDataIntoChunks(data);
      console.log(1, chunks);
      const failedChunks = [];

      for (const [index, chunk] of chunks.entries()) {
        console.log("holaaa", index, chunk);
  ...
  toSync = async () => {
    try {
      ...
      return this.#localProvider.store.filter(record => record.offline === true).toArray();
    } catch (e) {
      console.error(e);
    }
  };
