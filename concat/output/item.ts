 ...
 */
export /*bundle */ class RegistryFactory<T> extends ReactiveModel<
  RegistryFactory<T>
> {
  ...
  getItem(id: RegistryId, data: any): Registry {
    if (!id || !this.items.has(id)) {
      const specs = data
        ? { id, ...data }
        : { id, properties: this.properties, ...data };
      ...
      registry.on("record.updated", (registry) =>
        this.trigger("update.registry", registry)
      );
      registry.on("record.deleted", (registry) =>
